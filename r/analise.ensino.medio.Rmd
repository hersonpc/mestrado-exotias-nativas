...
grafico_densidade <- function(variavel, cor, titulo) {
    ...
    nomalidade <- 
        ggtexttable(
            normalidade(pos_gabarito$proporcoes[[variavel]]), 
...
q1 <- qqplot.data(pos_gabarito$proporcoes$p_nativas, "Identificação do nome das espécies de origem Nativas")
q2 <- qqplot.data(pos_gabarito$proporcoes$p_exoticas, "Identificação do nome das espécies de origem Exoticas")
q3 <- qqplot.data(pos_gabarito$proporcoes$p_origem_nativas, "Identificação de origem Nativas")
q4 <- qqplot.data(pos_gabarito$proporcoes$p_origem_exoticas, "Identificação de origem Exoticas")
q5 <- qqplot.data(pos_gabarito$proporcoes$p_indice_nativas, "Indice de reconhecimento Nativas (origens + espécies)")
q6 <- qqplot.data(pos_gabarito$proporcoes$p_indice_exoticas, "Indice de reconhecimento Exoticas (origens + espécies)")
...
variancia <- data.frame(
    ...
    Teste = c(
        "Indice de reconhecimento: Nativas x Exóticas",
        "Identificação do nome das espécies de origem: Nativas x Exóticas",
        "Identificação de origem: Nativas x Exóticas"
    ),
    "p_valor" = c(
        var.test(pos_gabarito$proporcoes$p_indice_nativas, 
                 pos_gabarito$proporcoes$p_indice_exoticas, 
                 alternative = "two.sided")$p.value,
        var.test(pos_gabarito$proporcoes$p_nativas, 
                 pos_gabarito$proporcoes$p_exoticas, 
                 alternative = "two.sided")$p.value,
        var.test(pos_gabarito$proporcoes$p_origem_nativas, 
                 pos_gabarito$proporcoes$p_origem_exoticas, 
                 alternative = "two.sided")$p.value
    )
)
...
```{r}
## Hipotese 1
boxplot(pos_gabarito$proporcoes$p_indice_nativas, pos_gabarito$proporcoes$p_indice_exoticas)
boxplot(pos_gabarito$proporcoes$p_nativas, pos_gabarito$proporcoes$p_exoticas)
boxplot(pos_gabarito$proporcoes$p_origem_nativas, pos_gabarito$proporcoes$p_origem_exoticas)

t.test(pos_gabarito$proporcoes$p_indice_nativas, 
       pos_gabarito$proporcoes$p_indice_exoticas, 
       alternative = "two.sided", paired = F, var.equal = T)
```

```{r}
t1 <- t.test(pos_gabarito$proporcoes$p_indice_nativas, 
       pos_gabarito$proporcoes$p_indice_exoticas, 
       alternative = "two.sided", paired = F, var.equal = T)
t2 <- t.test(pos_gabarito$proporcoes$p_nativas, 
       pos_gabarito$proporcoes$p_exoticas, 
       alternative = "two.sided", paired = F, var.equal = T)
t3 <- t.test(pos_gabarito$proporcoes$p_origem_nativas, 
       pos_gabarito$proporcoes$p_origem_exoticas, 
       alternative = "two.sided", paired = F, var.equal = T)
...
```{r}
## Hipotese 2
t.test(pos_gabarito$proporcoes$p_origem_nativas, 
       pos_gabarito$proporcoes$p_origem_exoticas, 
       alternative = "two.sided", paired = F, var.equal = T)
```
