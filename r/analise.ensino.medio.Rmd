...
```{r, fig.width=8, fig.height=5}
anova_taxonomico <- teste.anova(formula = value ~ grupo, 
            data = pos_gabarito$taxonomicos)

anova_taxonomico$teste.normalidade
anova_taxonomico$teste.anova
anova_taxonomico$tabela.tukey
...
nat <- NULL
nat$silvania <- 
    na.omit(spread(proporcoesPorMunicipio, key = municipio, value = p_nativas)[,"Silvânia"]) %>% as.vector()
nat$belavista <- 
    na.omit(spread(proporcoesPorMunicipio, key = municipio, value = p_nativas)[,"Bela Vista de Goiás"]) %>% as.vector()

boxplot(nat$silvania, nat$belavista)
#var.test(nat_silvania, nat_belavista, alternative = "two.sided")

t.test(nat$silvania, nat$belavista,
       alternative = "two.sided", paired = F, var.equal = T)
```
...
```{r}
aggregate(value ~ frequencia, data = tmp %>% filter(indicador == "indice"), FUN = function(x) shapiro.test(x)$p.value)
